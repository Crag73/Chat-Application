...
export const newMessage = async (req, res) => {
    ...
    if (!conversationId) {
        return res.status(400).json({ message: "Must provide a conversation Id" });
    }
    ...
    try {
        ...
        conversation?.participants.filter((participant) => participant.userId !== parsedAuthorId)
...
export const getMessagesInConversation = async (req,res) => {
    ...
    try {
        ...
        if (conversation?.participants[0].userId !== parsedCurrentUserId && conversation?.participants[1].userId !== parsedCurrentUserId) {
